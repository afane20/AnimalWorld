...
public class CounterManagerDificil : MonoBehaviour {
	...
	void Start () {
		levelName.text = this.GetComponent<AppManagerDificil> ().actualLevelName;
	...
	bool isOpen = false;
	void Update () {
		counterOfMatches = this.GetComponent<AppManagerDificil> ().totalMatches;
		...
		if (timeForThisLevel <= 0){
			if (lostRotateNumber) {
				...
				if (countRotations == 60) {
					...
					coinsEarned.text = "             " + PlayerPrefs.GetFloat(this.GetComponent<AppManagerDificil>().getKEYcoins);
					BestScore.text =   "Best:    " + PlayerPrefs.GetFloat(this.GetComponent<AppManagerDificil>().getKEYbestScore);
	...
	public void starsAnimation(){
		numberOfMatches = this.GetComponent<AppManagerDificil> ().totalMatches;
